...
import '../../../features/account/data/request/param/confirm_code_param.dart';
import '../../../features/account/data/request/param/confirm_delete_my_account_param.dart';
import '../../../features/account/data/request/param/confirm_new_phone_number_param.dart';
import '../../../features/account/data/request/param/resend_code_param.dart';
import '../../../features/account/domain/entity/confirm_code_entity.dart';
...
class __OTPDialogState extends State<_OTPDialog> {
  ...
  void _confirmCodeLoaded(ConfirmCodeEntity data) async {
    if (data.accessToken.isNotEmpty) {
      print('data.accessToken: ${data.accessToken}');
      print('data.refreshToken: ${data.refreshToken}');
      await LocalStorage.persistToken(data.accessToken, data.refreshToken);
      isLoading = false;
  ...
  void _onResendTap() {
    switch (widget.otpRequestType) {
      case OtpRequestType.ConfirmProfileNumberChange:
        accountCubit.resendNewPhoneNumberCode(NoParams());
        break;
      case OtpRequestType.ConfirmLoginOrRegister:
        accountCubit.resendCode(
          ResendCodeParam(
      ...
      case OtpRequestType.AccountDelete:
        accountCubit.resendDeleteMyAccountCode();
        break;
    }
  }
  ...
  void _onConfirmTap() {
    switch (widget.otpRequestType) {
      case OtpRequestType.ConfirmProfileNumberChange:
        accountCubit.confirmNewPhoneNumber(
          ConfirmNewPhoneNumberParam(confirmationCode: _otp),
        );
        break;
      case OtpRequestType.ConfirmLoginOrRegister:
        accountCubit.confirmCode(
          ConfirmCodeParam(
            phoneNumber: widget.phoneNumber,
            code: _otp,
      ...
      case OtpRequestType.AccountDelete:
        accountCubit.confirmDeleteMyAccount(
          ConfirmDeleteMyAccountParam(deletionCode: _otp),
        );
        break;
    }
  }
