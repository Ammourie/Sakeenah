...
import '../api/model/distance_result.dart';
import '../api/params/map_distance_params.dart';
import '../cubit/cubit.dart';
import '../logic/custom_map_model.dart';
import '../logic/custom_marker.dart';
import '../logic/distance_window_data.dart';
import '../logic/info_window_data.dart';
import '../logic/location_wrapper.dart';
...
class CustomMapState extends State<CustomMap> {
  ...
  /// as it moves
  Position? _currentLocation; // a reference to the destination location
  ...
  final locationWrapper = LocationWrapper.singleton();
  ...
  void initState() {
    super.initState();
    customMapModel = context.read<CustomMapModel>();

    _distanceWindowWidth = widget.distanceWindowWidth ?? 140.w;
    _distanceWindowHeight = widget.distanceWindowHeight ?? 150.h;

    customMapModel.setMapDimensions(widget.width, widget.height);

    polylinePoints = PolylinePoints(apiKey: widget.mapApiKey);

    ///delete old markers
    customMapModel.clear();
    _initializeMarkers();
  ...
  Widget build(BuildContext context) {
    final distanceLeftMargin =
        context.watch<CustomMapModel>().distanceLeftMargin ?? 0.0;
    final distanceTopMargin =
        context.watch<CustomMapModel>().distanceTopMargin ?? 0.0;
    final showDistanceWindow = context
        .watch<CustomMapModel>()
        .showDistanceWindow;
    final infoWindowLeftMargin = context
        .watch<CustomMapModel>()
        .infoWindowLeftMargin;
    final infoWindowTopMargin = context
        .watch<CustomMapModel>()
        .infoWindowTopMargin;
    final showInfoWindow = context.watch<CustomMapModel>().showInfoWindow;
      ...
      listener: (context, state) {
        state.map(
          ...
          mapLoading: (_) {
            ...
            customMapModel.updateDistanceWindowData(showWindow: false);
          },
          mapDistanceLoaded: (s) {
            _distanceWindowResult = DistanceResult(
              ...
              distanceM:
                  s.mapDistanceResult.rows[0].elements[0].distance!.value!,
              timeSec: s.mapDistanceResult.rows[0].elements[0].duration!.value!,
            );
            _updateDistanceWindowLocation(locations: polylineCoordinates);
            customMapModel.updateDistanceWindowData(showWindow: false);
          },
          mapError: (e) {
            ErrorViewer.showError(
              ...
              error: e.error,
              callback: e.callback,
              errorViewerOptions: const ErrVDialogOptions(isDismissible: true),
            );
          },
        );
      },
        ...
        child: Consumer<CustomMapModel>(
          builder: (context, model, child) {
            return Stack(
              children: [
                child!,
                Positioned(
                  left: distanceLeftMargin,
                  top: distanceTopMargin,
                  child: Visibility(
                    visible: showDistanceWindow,
                    child:
                        (distanceWindowData()?.windowWidget == null ||
                            !showDistanceWindow)
                ...
                Positioned(
                  left: infoWindowLeftMargin,
                  top: infoWindowTopMargin,
                  child: Visibility(
                    ...
                    child:
                        ...
                        : SizedBox(
                            height:
                                (_getCurrentMarker()!.tailHeight ?? 15) +
          ...
          child: Positioned(
            child: GoogleMap(
              ...
              markers: context.watch<CustomMapModel>().markers.toSet(),
  ...
  void onMapCreated(GoogleMapController controller) async {
    ...
    customMapModel.controller = controller;
    if (widget.initialIndex != null &&
      ...
      mapController!.animateCamera(
        CameraUpdate.newLatLng(widget.markers[widget.initialIndex].location!),
      );
    }
  }
  ...
  void setPolylines(
    ...
    try {
      ...
      ErrorViewer.showError(
        context: context,
        error: CustomError(message: S.current.connectionErrorMessage),
      ...
      ErrorViewer.showError(
        context: context,
        error: CustomError(message: S.current.mapPathError),
        callback: () => setMyLocationPolylines(
          // currentLocation,
          destinationLocation,
          context,
  ...
  void _unSelectCurrentMarker() {
    if (customMapModel.showInfoWindow) {
      customMapModel.updateInfoWindowData(showWindow: false);
    }
    ...
    if (_isMyLocationInfoWindowOpen && widget.myLocationMarker != null) {
      BitmapDescriptor icon = widget.myLocationMarker!.getMarkerIcon(
        isSelected: false,
      );
      customMapModel.addMarker(
        true,
        false,
  ...
  _changeInfoWindowAndMarker({
    ...
    customMapModel.updateInfoWindowData(
  ...
  ///
  _updateInfoWindowScreenCoordinates() {
    customMapModel.updateWindowScreenCoordinates(
      context,
  ...
  void onMarkerTap({
    ...
    _isMyLocationInfoWindowOpen = isMyLocationMarker == null
        ? false
        : isMyLocationMarker;

    /// if the selected marker is my Location marker then the current selected marker is null;
    currentSelectedMarkerIndex = isMyLocationMarker == true
        ? myLocationInfoWindowIndex
        : index;
    ...
    _changeInfoWindowAndMarker(
      customMarker: marker!,
      windowId: index!,
      isMyLocationInfoWindow: isMyLocationMarker == null
          ? false
          : isMyLocationMarker,
    );
  ...
  _updateDistanceWindowLocation({List<LatLng>? locations}) async {
    ...
    bool success = false;
    customMapModel.updateDistanceWindowData(
      windowWidth: _distanceWindowWidth,
      vOffset: _distanceWindowHeight,
      windowWidget: _buildDistanceWindow,
      notify: false,
    );
    if (customMapModel.showDistanceWindow) {
      success = await customMapModel.updateWindowScreenCoordinates(
        context,
    ...
    customMapModel.updateDistanceWindowData(showWindow: success);
  }
  ...
  Future<bool> _locationChangesListener() async {
    ...
    if (hasLocationPermissions) {
      ...
      _locationStream = locationWrapper.location.getPositionStream().listen((
        Position cLoc,
      ) {
