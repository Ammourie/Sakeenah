...
class FireBaseMessagingWrapper {
  ...
  Future<void> _initFirebaseMessaging() async {
    // request permission to show notifications (required in android 13 and higher)
    ...
    FirebaseMessaging.onMessageOpenedApp.listen(
      (message) => _onMessageTap(message: message),
    );
  ...
  void _pushLocalNotification(RemoteMessage message) {
    ...
    _streamController.add(NotificationData());
  }
  ...
  static AddOrUpdateFirebaseTokenParam getUpdateTokenParam() {
    assert(
      AppConfig().deviceId != null && AppConfig().os != null,
      "deviceId or os is null",
    );
    return AddOrUpdateFirebaseTokenParam(
      deviceId: AppConfig().deviceId!,
      deviceType: AppConfig().os!,
      newToken: LocalStorage.fcmToken,
      oldToken: LocalStorage.oldFcmToken,
    );
  }

  static AddOrUpdateFirebaseTokenParam getDeleteTokenParam() {
    assert(
      AppConfig().deviceId != null && AppConfig().os != null,
      "deviceId or os is null",
    );
