  ...
  HttpClient() {
    ...
    if (AppSettings.enableDioPrinting) {
      _client.interceptors.add(
        PrettyDioLogger(requestBody: true, maxWidth: 270),
      );
    }
  ...
  Future<Either<AppErrors, T>> sendRequest<T extends BaseModel>({
    ...
    try {
      ...
      if (responseValidator.isValid) {
        ...
        return Left(CustomError(message: S.current.generalErrorMessage));
    }
    /// Handling errors
  ...
  AppErrors _handleDioError<E>(DioException error) {
    if (error.type == DioExceptionType.unknown ||
          ...
          case 500:
            if (error.response?.data is Map) {
              if (error.response!.data?["error"]?["message"] != null ||
                  error.response!.data?["error"]?["errorCode"] != null) {
                final errorCode =
                    int.tryParse(
      ...
      if (error.type == DioExceptionType.connectionTimeout ||
        ...
        return CancelError(S.current.cancelErrorMessage);
      } else
        return const UnknownError();
    }
  }
