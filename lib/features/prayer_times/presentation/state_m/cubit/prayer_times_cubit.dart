...
class PrayerTimesCubit extends Cubit<PrayerTimesState> {
  ...
  Future<void> initialize({
    ...
    if (_currentLocation != null && _currentLocation!.isValid) {
      ...
      await _refreshPlaceLabelForLocaleIfNeeded();
  ...
  Future<void> refresh({required bool hasInternet}) async {
  ...
  Future<void> refreshPlaceLabelForCurrentLocale() async {
    ...
    final previousLabel = _currentLocation!.displayLabel;
    ...
    if (_currentLocation == null ||
        ...
        _currentLocation!.displayLabel == previousLabel) {
    ...
    state.maybeWhen(
      ...
      loaded: (schedule, location, isCached) {
  ...
  Future<void> saveMapLocation({
    ...
    if (resolved != null) {
      ...
      log(
        ...
        'saveMapLocation geocode failed — saving GPS coords without label',
  ...
  Future<bool> _applyPrefetchedGps(LocationPreferenceEntity? prefetchedGps) async {
  ...
  Future<void> _refreshPlaceLabelForLocaleIfNeeded() async {
    ...
    final appLanguage = AppConfig().appLanguage.languageCode;

    final label = current.displayLabel.trim();

    final hasReadableLabel =

        label.isNotEmpty && !LocationLabelUtils.isCoordinateLabel(label);

    final localeMatches = current.labelLanguageCode == appLanguage;

    if (hasReadableLabel && localeMatches) {

      log(

        '_refreshPlaceLabelForLocaleIfNeeded skip — label up to date: $label',
    ...
    log(
      ...
      '_refreshPlaceLabelForLocaleIfNeeded refreshing label for '

      '(${current.latitude},${current.longitude}) locale=$appLanguage',
    ...
    if (resolved == null) {
      ...
      log('_refreshPlaceLabelForLocaleIfNeeded geocode returned null');
    ...
    final updated = LocationPreferenceEntity(
    ...
    final saveResult = await _saveLocationPreference(
      ...
      SaveLocationPreferenceParams(location: updated),

    );

    if (saveResult.hasErrorOnly) return;
    ...
    result.pick(
      ...
      onError: (error) {
        ...
        _emitError(
          ...
          () => refresh(hasInternet: hasInternet),
  ...
  void _emitError(AppErrors error, VoidCallback callback) {
    ...
    safeEmit(
      ...
      PrayerTimesState.error(error: error, callback: callback),
